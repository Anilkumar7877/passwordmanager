...
const manager = () => {
  ...
  const savepassword = async (params) => {
    if (form.site[0].length > 3) {
      ...
      //to save the password, method : post
  ...
  const handleedit = (id) => {
    // console.log(id)
    setform({...passwordsArray.filter(item => item.id === id)[0], id:id})//codewithharry logic
    //mylogic
              ...
              <tbody className='bg-slate-200'>
                {passwordsArray.map(item=>{
                  ...
                  <tr key={item.id}>
                    ...
                    <td className='text-center py-1'>
                      <div className='flex gap-4 justify-center py-2'>
                        <a href={item.site} target="_blank">{"*".repeat(item.password[0].length)}</a>
